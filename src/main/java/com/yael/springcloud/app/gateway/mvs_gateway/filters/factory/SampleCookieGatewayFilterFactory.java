package com.yael.springcloud.app.gateway.mvs_gateway.filters.factory;
...
public class SampleCookieGatewayFilterFactory extends AbstractGatewayFilterFactory<SampleCookieGatewayFilterFactory.ConfigurationCookie> {
    ...
    public GatewayFilter apply(ConfigurationCookie config) {
        return new OrderedGatewayFilter(
            (exchange, chain) -> {
                // Pre
                logger.info("Ejecutando pre gateway factory " + config.message);
                ...
                return chain.filter(exchange)
                    .then(Mono.fromRunnable( () -> {
                        ...
                        logger.info("Ejecutando post gateway factory " + config.message);
                    }));
            }, 100
        );
    }
